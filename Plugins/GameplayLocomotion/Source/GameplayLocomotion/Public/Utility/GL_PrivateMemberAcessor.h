...
namespace GameplayPrivateMemberAccessor
{
...
#define GL_DEFINE_PRIVATE_MEMBER_ACCESSOR(AccessorName, MemberPointer, ...) \
	struct AccessorName \
	{ \
		...
		{ \
			return GameplayPrivateMemberAccessor::Access<AccessorName>(Forward<ThisType>(This)); \
		} \
		\
		template <typename ThisType, typename... ArgumentsType> \
		static decltype(auto) Invoke(ThisType&& This, ArgumentsType&&... Arguments) \
		{ \
			return GameplayPrivateMemberAccessor::Invoke<AccessorName>(Forward<ThisType>(This), Forward<ArgumentsType>(Arguments)...); \
		} \
	}; \
	\
	template struct GameplayPrivateMemberAccessor::TPointerInitializer<AccessorName, MemberPointer>;
